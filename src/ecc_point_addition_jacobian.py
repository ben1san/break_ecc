...
def ecc_point_addition_jacobian(p_regs, q_regs, ancilla_pool, N_mod):
    ...
    # 公式計算に必要な中間変数
    Z1_sq = ancilla_pool[0]  # Z1^2
    Z2_sq = ancilla_pool[1]  # Z2^2
    ...
    # 1-1. Z2^2 を計算
    mod_sq(qc, Z2, Z2_sq, N_mod)
    
    # 1-2. U1 = X1 * Z2^2
    mod_mult(qc, X1, Z2_sq, U1, N_mod)
    
    # --- Step 2: U2 = X2 * Z1^2 の計算 ---
    
    # 2-1. Z1^2 を計算
    mod_sq(qc, Z1, Z1_sq, N_mod)
    
    # 2-2. U2 = X2 * Z1^2
    mod_mult(qc, X2, Z1_sq, U2, N_mod)
    ...
    # H = U2 - U1
    mod_sub(qc, U1, U2, N_mod) # 結果は U2レジスタ(H) に残る
    ...
    mod_mult(qc, X1, Z2_sq, U1, N_mod).inverse() # U1の計算を巻き戻す
    mod_sq(qc, Z2, Z2_sq, N_mod).inverse()       # Z2^2の計算を巻き戻す
