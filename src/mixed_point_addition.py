...
def mixed_point_addition(qc, q_point,  c_point, ancilla_pool, N_mod):
    """
    ...
    # 1. A = Z1^2
    ops.mod_square(qc, Z1, Z1_sq, N_mod)

    # 2. B = Z1 * A (= Z1^3)
    ops.mod_multiply(qc, Z1, Z1_sq, Z1_cub, N_mod)

    # 3. U2 = x2 * A (定数乗算)
    # ops.const_mult(circuit, target, const_val, output)
    ops.mod_mult_const(qc, Z1_sq, x2, U2, N_mod)

    # 4. S2 = y2 * B (定数乗算)
    ops.mod_mult_const(qc, Z1_cub, y2, S2, N_mod)
    ...
    ops.mod_copy(qc, U2, H) # U2をHにコピー
    ops.mod_sub(qc, X1, H, N_mod)

    # 6. R = S2 - Y1
    ops.mod_copy(qc, S2, R)
    ops.mod_sub(qc, Y1, R, N_mod)
    ...
    ops.mod_multiply(qc, Z1, H, Temp_Z3, N_mod)

    # --- Phase 4: X3 の計算 ---
    
    # H^2, H^3 の準備
    ops.mod_square(qc, H, H_sq, N_mod)
    ops.mod_multiply(qc, H, H_sq, H_cub, N_mod)
    ...
    ops.mod_square(qc, R, Term1, N_mod)
    
    # Term2 = 2 * X1 * H^2
    Term2 = ancilla_pool[10]
    ops.mod_multiply(qc, X1, H_sq, Term2, N_mod)
    ops.mod_add(qc, Term2, Term2, N_mod) # 2倍 (自己加算)
    
    # X3_new (Temp) = Term1 - H_cub - Term2
    X3_new = ancilla_pool[11]
    ops.mod_copy(qc, Term1, X3_new)
    ops.mod_sub(qc, H_cub, X3_new, N_mod)
    ops.mod_sub(qc, Term2, X3_new, N_mod)
    ...
    # 例: Hの計算に使ったリソースを解放
    ops.mod_sub(qc, X1, H, N_mod).inverse() 
    # ... 全てを逆順で実行 ...
